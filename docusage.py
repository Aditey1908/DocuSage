"""
DocuSage API - Combined Flask Application

This application provides both the original document processing API 
and the newer thread-based API for document chat.
"""
...
def create_thread_endpoint():
    ...
    try:
        ...
        persistent_dir = os.path.join(DATA_DIR, 'thread_documents')
...
def process_message_with_rag(thread_id, question, idempotency_key, memory_version=None, parent_message_id=None):
    ...
    # Use temporary file for question
    with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json') as temp_file:
        ...
        json.dump([enhanced_question], temp_file)
        questions_path = temp_file.name
